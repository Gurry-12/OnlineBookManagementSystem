...
$(document).on('click', '#SubmitForm', async function (e) {
    ...
    const email = $('#Email').val().trim();
    const password = $('#Password').val().trim();
    const requestedRole = $('input[name="RequestedRole"]:checked').val() || 'User';
    const acceptTerms = $('#AcceptTerms').is(':checked');
    const subscribeToUpdates = $('#SubscribeToUpdates').is(':checked');
    ...
    if (!acceptTerms) {
        toastr.error('Please accept the Terms of Service and Privacy Policy to continue.');
        isValid = false;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            const roleMessage = requestedRole === 'Admin'
                ? 'Admin account created! Please check your email and wait for approval.'
                : 'Account created successfully! Please check your email to verify your account.';

            toastr.success(roleMessage);

            // Show additional onboarding information
            showOnboardingSuccess(requestedRole);
...
function showOnboardingSuccess(role) {
    const messages = {
        'User': {
            title: 'Welcome to Whispering Pages!',
            message: 'Your account has been created. Please check your email to verify your account and start exploring our book catalog.',
            nextSteps: [
                'Check your email for verification link',
                'Complete email verification',
                'Start browsing our book collection'
            ]
        },
        'Admin': {
            title: 'Admin Account Requested',
            message: 'Your admin account request has been submitted. You will receive an email once approved by our team.',
            nextSteps: [
                'Check your email for verification link',
                'Wait for admin approval (24-48 hours)',
                'Access your admin dashboard once approved'
            ]
        }
    };

    const info = messages[role] || messages['User'];
    ...
    toastr.info(`
        ...
        ${info.nextSteps.map(step => `• ${step}`).join('<br>')}
    `, '', {
        allowHtml: true,
        closeButton: true
    });
