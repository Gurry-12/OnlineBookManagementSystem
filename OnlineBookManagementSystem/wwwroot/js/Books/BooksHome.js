...
// Modern, clean, works with JWT + new auth system
toastr.options = {
    closeButton: true,
    progressBar: true,
...
// Utility: Get token from HttpOnly cookie (fallback)
...
// Get user info from localStorage (set on login)
...
function updateUIBasedOnRoles() {
    ...
    // Hide/show admin/superadmin links
...
async function AddToCart(bookId, quantity = 1) {
    ...
    try {
        const response = await fetch('/Cart/AddOrUpdateCart', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            updateCartCount(); // optional: refresh badge
        } else {
            toastr.error(result.message || 'Out of stock!');
        }
    } catch (err) {
...
async function ToggleFavorite(bookId) {
    ...
    try {
        ...
        if (result.success) {
            if (isFav) {
                btn.removeClass('text-danger').html('<i class="bi bi-heart"></i>');
                toastr.info('Removed from favorites');
            } else {
                btn.addClass('text-danger').html('<i class="bi bi-heart-fill"></i>');
...
async function UpdateQuantity(bookId, change) {
    ...
    const success = await AddToCart(bookId, qty);
    if (success) input.val(qty);
}
