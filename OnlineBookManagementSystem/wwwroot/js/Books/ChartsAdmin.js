...
function setupChartRefresh() {
    ...
    // Auto-refresh every 5 minutes with new random data
    setInterval(() => {
        console.log('Auto-refreshing charts with updated seed data...');
        updateSeedDataWithRandomVariations();
        loadAllChartData();
    }, 300000);
}

// Function to add some random variations to seed data for demo purposes
function updateSeedDataWithRandomVariations() {
    // Add small random variations to monthly data
    seedData.monthly.forEach(item => {
        const variation = Math.floor(Math.random() * 10) - 5; // -5 to +5
        item.count = Math.max(1, item.count + variation);
    });

    // Add small random variations to revenue data
    seedData.revenue.forEach(item => {
        const variation = (Math.random() * 1000) - 500; // -500 to +500
        item.revenue = Math.max(100, item.revenue + variation);
    });

    console.log('Seed data updated with random variations');
}